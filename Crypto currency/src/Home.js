import React from "react";
import { bindActionCreators } from "redux";
import { connect } from "react-redux";
import _ from 'lodash';
import { requestApiData } from "./actions";


class Home extends React.Component {
  componentDidMount() {
    this.props.requestApiData();
  }
  coindata = (x) =>
  <table style={{ tableLayout: 'auto' }}>
                                    <colgroup>
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                    <col style={{ width: '240px', minWidth: '240px' }} />
                                  </colgroup>
      <tbody class="ant-table-tbody" key={x.coin}>

        <tr class="ant-table-row ant-table-row-level-0 pointer">
          <td class="ant-table-cell text-left">
            <div style={{ display: 'flex', justifyContent: 'center' }}>
            
              <div style={{ width: '45px', textAlign: 'left' }}>

                <img alt="BTC" src={x.icon_image} style={{ backgroundImage: 'url(&quot;data:image/png;base64,&quot;)', height: '31px', width: '31px', backgroundSize: 'contain', marginRight: '20px', display: 'block', backgroundRepeat: 'no-repeat' }}>
                </img>
              </div>
              <div style={{ width: '70px', textAlign: 'left', color: 'rgb(0, 0, 0)', marginTop: '3px' }}>
              {x.symbol}
              </div>
              <div style={{ width: '230px', textAlign: 'left', fontSize: '18px', lineHeight: '28px', color: 'rgb(118, 128, 143)', marginTop: '3px' }}>
              {x.name}
              </div>
            
            </div>
          </td>

          <td class="ant-table-cell text-left">
            {x.last_price}
             {/* last price */}
          </td>

          <td class="ant-table-cell text-left">
            <div style={{ color: 'rgb(207, 19, 34)' }}>{x.change} 
            {/* change */}
            </div>
          </td>
          <td class="ant-table-cell text-left">
            <div style={{ color: 'rgb(207, 19, 34)' }}>{x.high} 
            {/* high */}
            </div>
          </td>
          <td class="ant-table-cell text-left">
            <div style={{ color: 'rgb(207, 19, 34)' }}>{x.low} 
            {/* low */}
            </div>
          </td>
          <td class="ant-table-cell text-left">
            <div>{x.volume} 
            {/* volume */}
            </div>
          </td>
        </tr>
      </tbody>
 </table>


  render() { 
    const { data = [] } = this.props.data
    // const groups = _.props.groupBy(data,'name')

    // console.log(groups)
    
   
    return data.length?
     <div>
          {data.map(this.coindata)}
      </div>
        
      : <h1>not good...</h1>;
      
  }
   }


const mapStateToProps = state => ({ data: state.data });
const mapDispatchToProps = dispatch =>
  bindActionCreators({ requestApiData }, dispatch);

export default connect(mapStateToProps, mapDispatchToProps)(Home);
   